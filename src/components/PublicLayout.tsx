        ...
        <div className="container flex h-[62px] flex-nowrap items-center justify-between gap-2 sm:gap-5">
          <Link to="/home" className="flex min-w-0 items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-[8px] font-semibold uppercase tracking-[0.08em] text-primary/70 sm:text-[10px] sm:tracking-[0.16em]">
          ...
          <div className="hidden items-center gap-5 lg:flex">
            <nav className="flex items-center gap-5">
              ...
              <Link to="/admissions" className="text-xs font-semibold text-muted-foreground transition-colors hover:text-foreground">{t('Admissions', 'القبول')}</Link>
          ...
          <div className="flex shrink-0 items-center gap-2 lg:hidden"><button type="button" onClick={() => setMobileOpen(true)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-border text-foreground" aria-label={t('Open menu', 'فتح القائمة')}><Menu className="h-5 w-5" /></button></div>
        </div>
      </header>
      {mobileOpen && <div className="fixed inset-0 z-50 bg-background lg:hidden"><div className="flex h-[76px] items-center justify-between border-b border-border px-5"><span className="font-display text-lg font-extrabold">{t('Menu', 'القائمة')}</span><button type="button" onClick={() => setMobileOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-lg border border-border" aria-label={t('Close menu', 'إغلاق القائمة')}><X className="h-5 w-5" /></button></div><nav className="flex flex-col gap-1 p-5">{[["/home", 'Home'], ['/about', 'About Us'], ['/academics', 'Academics'], ['/news-events', 'News & Events'], ['/gallery', 'Gallery'], ['/contact', 'Contact'], ['/results', 'Check Result']].map(([path, label]) => <Link key={path} to={path} onClick={() => setMobileOpen(false)} className="border-b border-border py-4 text-base font-bold">{t(label, label)}</Link>)}<Link to="/admissions" onClick={() => setMobileOpen(false)} className="border-b border-border py-4 text-base font-bold text-primary">{t('Admissions', 'القبول')}</Link><div className="mt-6 grid gap-3"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{t('Portals', 'البوابات')}</p><Link to="/admin/login" onClick={() => setMobileOpen(false)} className="rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground">{t('Staff Portal', 'بوابة الموظفين')}</Link></div><div className="mt-6"><LanguageToggle /></div></nav></div>}
