    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <footer className="bg-[hsl(var(--sidebar-background))] py-12 text-[hsl(var(--sidebar-foreground))]">
        <div className="container grid gap-10 md:grid-cols-4">
          ...
          <div><p className="mb-4 text-xs font-bold uppercase tracking-[0.16em] text-accent">{t('Portals', 'البوابات')}</p><div className="grid gap-3 text-sm text-sidebar-foreground/70"><Link to="/results" className="hover:text-accent">{t('Result Checker', 'الاستعلام عن النتائج')}</Link><Link to="/staff/signup" className="hover:text-accent">{t('Staff Portal', 'بوابة الموظفين')}</Link><Link to="/admin/login" className="hover:text-accent">{t('Admin Portal', 'بوابة المسؤول')}</Link></div></div>
