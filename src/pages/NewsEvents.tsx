...
const NewsEvents = () => {
  ...
  const events = settings['website.events'].split('|').filter(Boolean);
      ...
      <main className="container py-20">
        <section>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {articles.map((article, index) => <article key={article.title} className={`group flex min-h-[270px] flex-col justify-between border border-border bg-card p-6 shadow-card transition-transform hover:-translate-y-1 ${index === 0 ? 'lg:border-t-4 lg:border-t-accent' : ''}`}>
              <div><div className="flex items-center justify-between gap-3 text-[10px] font-bold uppercase tracking-[0.15em] text-primary"><span>{article.category}</span><Megaphone className="h-4 w-4" /></div><h3 className="mt-10 font-display text-2xl font-extrabold leading-tight">{article.title}</h3></div>
              <div className="mt-8 flex items-center justify-between border-t border-border pt-4 text-xs text-muted-foreground"><time>{article.date}</time><ChevronRight className="h-4 w-4 text-primary transition-transform group-hover:translate-x-1" /></div>
            </article>)}
          </div>
        </section>
        <section className="mt-24 grid gap-10 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <div className="divide-y divide-border border-y border-border">{events.map((event, index) => <article key={event} className="flex items-center gap-5 py-5"><div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center bg-primary text-primary-foreground"><CalendarDays className="mb-1 h-4 w-4" /><span className="text-[10px] font-bold uppercase">{dates[index] || t('Soon', 'قريباً')}</span></div><div><p className="font-display text-xl font-bold">{event}</p><p className="mt-1 text-xs text-muted-foreground">{t('Institute event', 'فعالية المعهد')}</p></div></article>)}</div>
        </section>
      </main>
    </PublicLayout>
  );
