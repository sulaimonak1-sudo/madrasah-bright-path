    ...
    <PublicLayout>
      ...
      <main className="container py-20">
        ...
        <section className="mt-24 grid gap-10 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <div className="divide-y divide-border border-y border-border">{events.map((event, index) => <article key={('id' in event ? event.id : '') || event.title} className="flex items-center gap-5 py-5"><div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center bg-primary text-primary-foreground"><CalendarDays className="mb-1 h-4 w-4" /><span className="text-[10px] font-bold uppercase">{event.event_date || dates[index] || t('Soon', 'قريباً')}</span></div><div><p className="font-display text-xl font-bold">{event.title}</p>{event.excerpt && <p className="mt-1 text-sm text-muted-foreground">{event.excerpt}</p>}<p className="mt-1 text-xs text-muted-foreground">{t('Institute event', 'فعالية المعهد')}</p></div></article>)}</div>
        </section>
      </main>
    </PublicLayout>
  );
