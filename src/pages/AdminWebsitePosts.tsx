...
const AdminWebsitePosts = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const update = (index: number, field: keyof Post, value: string | null) => setPosts(current => current.map((post, itemIndex) => itemIndex === index ? { ...post, [field]: value } : post));
  useEffect(() => { supabase.from('website_posts').select('*').order('type').order('sort_order').order('published_at', { ascending: false }).then(({ data }) => { setPosts((data || []).map(post => ({ ...post, event_date: post.event_date || '', published_at: post.published_at?.slice(0, 16) || '' })) as Post[]); setLoading(false); }); }, []);
  const save = async (post: Post) => { setSaving(true); const { data, error } = await supabase.from('website_posts').upsert({ ...post, event_date: post.type === 'event' ? post.event_date || null : null, published_at: post.published_at ? new Date(post.published_at).toISOString() : null }).select().single(); if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' }); else if (data) setPosts(current => current.map(item => item.id === post.id ? { ...data, event_date: data.event_date || '', published_at: data.published_at?.slice(0, 16) || '' } as Post : item)); setSaving(false); };
  const remove = async (post: Post) => { if (!post.id) return setPosts(current => current.filter(item => item !== post)); const { error } = await supabase.from('website_posts').delete().eq('id', post.id); if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' }); else setPosts(current => current.filter(item => item.id !== post.id)); };
  const uploadImage = async (index: number, file: File) => { const path = `${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, '-')}`; const { error } = await supabase.storage.from('gallery').upload(path, file); if (error) return toast({ title: 'Upload failed', description: error.message, variant: 'destructive' }); update(index, 'image_url', supabase.storage.from('gallery').getPublicUrl(path).data.publicUrl); };
  return <AdminLayout><div className="mx-auto max-w-6xl space-y-6 pb-20"><header className="flex items-end justify-between border-b border-border pb-6"><div><p className="eyebrow text-primary">{t('Website management', 'إدارة الموقع')}</p><h1 className="mt-2 font-display text-3xl font-extrabold">{t('News & Events', 'الأخبار والفعاليات')}</h1></div><div className="flex gap-2"><Button variant="outline" onClick={() => setPosts(current => [emptyPost('event'), ...current])}><Plus className="mr-2 h-4 w-4" />{t('Add event', 'إضافة فعالية')}</Button><Button onClick={() => setPosts(current => [emptyPost('news'), ...current])}><Plus className="mr-2 h-4 w-4" />{t('Add news', 'إضافة خبر')}</Button></div></header>{loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin" /></div> : <div className="space-y-5">{posts.map((post, index) => <article key={post.id || index} className="rounded-xl border border-border bg-card p-5 shadow-card"><div className="mb-4 flex justify-between"><span className="text-xs font-bold uppercase text-primary">{post.type}</span><button onClick={() => remove(post)} aria-label="Delete post"><Trash2 className="h-4 w-4" /></button></div><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2 md:col-span-2"><Label>Title</Label><input value={post.title} onChange={event => update(index, 'title', event.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" /></div><div className="space-y-2"><Label>Category</Label><input value={post.category} onChange={event => update(index, 'category', event.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" /></div><div className="space-y-2"><Label>Publish date</Label><input type="datetime-local" value={post.published_at} onChange={event => update(index, 'published_at', event.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" /></div>{post.type === 'event' && <div className="space-y-2"><Label>Event date</Label><input type="date" value={post.event_date} onChange={event => update(index, 'event_date', event.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" /></div>}<div className="space-y-2 md:col-span-2"><Label>Excerpt</Label><textarea value={post.excerpt} onChange={event => update(index, 'excerpt', event.target.value)} className="min-h-[80px] w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" /></div><div className="space-y-2 md:col-span-2"><Label>Blog content</Label><textarea value={post.body} onChange={event => update(index, 'body', event.target.value)} className="min-h-[220px] w-full rounded-lg border border-input bg-background px-3 py-2 text-sm leading-7" /></div><div className="space-y-2 md:col-span-2"><Label>Blog image</Label><input type="file" accept="image/*" onChange={event => { const file = event.target.files?.[0]; if (file) uploadImage(index, file); }} /><p className="text-xs text-muted-foreground">Upload an image for this story.</p>{post.image_url && <img src={post.image_url} alt="" className="mt-2 h-32 w-48 object-cover" />}</div></div><div className="mt-4 flex justify-end"><Button size="sm" onClick={() => save(post)} disabled={saving || !post.title.trim()}><Save className="mr-2 h-4 w-4" />Save post</Button></div></article>)}</div>}</div></AdminLayout>;
};
export default AdminWebsitePosts;