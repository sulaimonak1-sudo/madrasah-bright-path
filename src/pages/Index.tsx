...
const Index = () => {
  ...
  return (
    <PublicLayout>
      ...
      <section className="relative min-h-[470px] overflow-hidden bg-[hsl(var(--sidebar-background))] text-primary-foreground md:min-h-[500px]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(90deg,hsl(var(--sidebar-background)/0.99)_0%,hsl(var(--sidebar-background)/0.96)_38%,hsl(var(--sidebar-background)/0.4)_75%,hsl(var(--sidebar-background)/0.18)_100%)]" />
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="mt-9 flex flex-wrap gap-3"><a href="#about" className="inline-flex h-12 items-center gap-2 bg-accent px-5 text-sm font-bold text-accent-foreground transition-transform hover:-translate-y-0.5">{t('Explore our institute', 'اكتشف معهدنا')} <ChevronRight className="h-4 w-4" /></a><a href={ADMISSIONS_URL} target="_blank" rel="noopener noreferrer" className="inline-flex h-12 items-center gap-2 border border-primary-foreground/30 px-5 text-sm font-bold text-primary-foreground hover:bg-primary-foreground/10">{t('Apply now', 'قدم الآن')} <ArrowRight className="h-4 w-4" /></a><Link to="/results" className="inline-flex h-12 items-center gap-2 bg-primary-foreground px-5 text-sm font-bold text-primary hover:bg-primary-foreground/90">{t('Check Madrasah Result', 'تحقق من نتيجة المدرسة')} <FileSearch className="h-4 w-4" /></Link></div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="container relative z-10 -mt-10 pb-20"><div className="ml-auto flex max-w-xl items-center justify-between gap-5 border border-border/70 bg-card p-5 shadow-card-lg"><div><p className="font-display text-lg font-extrabold">{t('Admissions are open', 'التسجيل مفتوح')}</p><p className="mt-1 text-sm text-muted-foreground">{t('Start your application online in a few minutes.', 'ابدأ طلب الالتحاق عبر الإنترنت في دقائق.')}</p></div><a href={ADMISSIONS_URL} target="_blank" rel="noopener noreferrer" className="inline-flex h-11 shrink-0 items-center gap-2 bg-primary px-4 text-xs font-bold text-primary-foreground hover:bg-primary/90">{t('Apply now', 'قدم الآن')} <ArrowRight className="h-4 w-4" /></a></div></section>

      <section className="container pb-20"><div className="flex flex-col items-center justify-between gap-6 rounded-2xl border border-border bg-muted/45 p-8 sm:flex-row sm:p-10"><div className="text-center sm:text-left"><p className="font-display text-2xl font-extrabold">{t('Looking for a student report?', 'هل تبحث عن تقرير طالب؟')}</p><p className="mt-2 text-sm text-muted-foreground">{t('Enter the student PIN to view, print or download the result slip.', 'أدخل رقم PIN الخاص بالطالب لعرض أو طباعة أو تنزيل نتيجته.')}</p></div><Link to="/results" className="inline-flex h-12 shrink-0 items-center gap-2 bg-primary px-6 text-sm font-bold text-primary-foreground hover:bg-primary/90">{t('Check Madrasah Result', 'تحقق من نتيجة المدرسة')} <FileSearch className="h-4 w-4" /></Link></div></section>
